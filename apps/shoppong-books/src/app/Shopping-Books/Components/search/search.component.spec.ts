...
describe('SearchComponent', () => {
  ...
  it('clicking on cart icon it should remove book from cart', inject(
    [BooksFacade],
    (facade : BooksFacade) => {
    const searchBtnClickedSpy = spyOn(component, 'getBooks').and.callThrough();
      const allBooksFacadeSpy = spyOn(facade, 'searchedBook');
      const id = "eNExy_X1YYcC";

    component.getBooks(id);
    fixture.detectChanges();

    expect(searchBtnClickedSpy).toHaveBeenCalled();
    expect(allBooksFacadeSpy).toHaveBeenCalled();
    }));

  it('clicking on Proceed to buy should navigate to billingpage page', async () => {
      const id = "eNExy_X1YYcC";
      component.onBookSelected(id);
      expect(mockRouter.navigate).toHaveBeenCalledWith(['/bookdetails/',id]);
    });
});
